...
\begin{tikzpicture}[
    ...
    % ---- R2 constant mux (new!) ----
    \node[constbox] (constmux) at (5.5, -0.8) {1 / 1665};
    ...
    \draw[arr] (r2mux.south) -- (r2mux.south |- constmux.north)
        node[lbl, midway, right] {12b};
    ...
    % R2 data to output muxes (R2 path)
    \draw[arr] (constmux.south) -- ++(0, -0.2) -| (outmux0.north east)
        node[lbl, pos=0.15, right] {$\omega$};
    \draw[arr] (constmux.south) -- ++(0, -0.35) -| ([xshift=2mm]outmux1.north east)
        node[lbl, pos=0.15, right] {$1$/$1665$};
    \draw[arr] (constmux.south) -- ++(0, -0.5) -| ([xshift=2mm]outmux2.north east)
        node[lbl, pos=0.12, right] {$\omega$};
    ...
    % is_intt to constant/omega4
    \draw[darr] (8.2, -0.8) -- (constmux.east) node[lbl, midway, above] {\port{is\_intt}};
...
\begin{tikzpicture}[
    ...
    \node[state, fill=nttblue!12]  (p1)   at (3.5,0)    {S\_PASS\_1};
    \node[state, fill=nttblue!18]  (p2)   at (7,0)      {S\_PASS\_2};
    \node[state, fill=nttblue!24]  (p3)   at (10.5,0)   {S\_PASS\_3};
    \node[state, fill=nttblue!30]  (p4)   at (10.5,-2.8){S\_PASS\_4};

    % Forward transitions
    \draw[arr] (idle) -- (p1) node[cond, midway, above] {\port{start\_i}};
    \draw[arr] (p1)   -- (p2) node[cond, midway, above] {\textit{pass\_done}};
    \draw[arr] (p2)   -- (p3) node[cond, midway, above] {\textit{pass\_done}};
    \draw[arr] (p3)   -- (p4) node[cond, midway, right] {\textit{pass\_done}};

    % Return to idle
    \draw[arr] (p4.south) -- ++(0,-1) -| (idle.south)
        node[cond, near start, right] {\textit{pass\_done}};

    % CWM shortcut
    \draw[arr, dashed, cwmgreen, thick]
        (p1.south) -- ++(0,-0.9) -| ([xshift=3mm]idle.south)
        node[cond, pos=0.25, right] {\textit{CWM done}};

    % Self loops
    \draw[arr, gray!60] (p1.north) to[loop above] node[cond] {counting} (p1.north);
    \draw[arr, gray!60] (p2.north) to[loop above] node[cond] {counting} (p2.north);
    \draw[arr, gray!60] (p3.north) to[loop above] node[cond] {counting} (p3.north);
    \draw[arr, gray!60] (p4.east)  to[loop right] node[cond] {counting} (p4.east);

\end{tikzpicture}
\caption{FSM state diagram of \sv{tf\_addr\_gen}. CWM mode exits after Pass~1.
  Each state self-loops while counting butterflies/blocks within its pass.}
\label{fig:fsm}
\end{figure}
...
\begin{tikzpicture}[
    ...
    \draw[arr] ([yshift=-4mm]ctrl.east) -- node[wire, above] {mode\_i}
               ([yshift=-4mm]addr.west);

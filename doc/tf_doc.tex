...
\begin{tikzpicture}[
    ...
    % Pass type annotations below each state
    \node[cyc, below=1mm of p1] {NTT: R4\,(64\,CC) / INTT: R2\,(128\,CC)};
    \node[cyc, below=1mm of p2] {R4\,(64\,CC)};
    \node[cyc, below=1mm of p3] {R4\,(64\,CC)};
    \node[cyc, below=1mm of p4] {NTT: R2\,(128\,CC) / INTT: R4\,(64\,CC)};
    ...
    \draw[arr, dashed, cwmgreen, thick]
        ...
        node[cond, pos=0.25, right] {\textit{CWM done}\; (64\,CC)};
    ...
    % Total cycle annotation
    \node[font=\scriptsize\sffamily, text=gray, anchor=north west] at (-1, -4.2)
        {Total: 320\,CC (NTT/INTT) or 64\,CC (CWM)};
...
\caption{FSM state diagram of \sv{tf\_addr\_gen}. CWM mode exits after Pass~1.
  ...
  Cycle counts (CC) are shown per pass.}
\label{fig:fsm}
\end{figure}
...
The Radix-4 ROM counter is \emph{not} reset between passes. It starts at 0 when \port{start\_i} fires and increments at \emph{every} block boundary (including the last block of each pass), counting continuously from 0 to 20 across all three Radix-4 passes:
\begin{itemize}
